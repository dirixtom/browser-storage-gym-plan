...
//
// buildApp() below only wires plugins/routes and takes its dependencies
// (pool, config) as arguments — this is what lets tests construct the app
// against a test Postgres via Fastify's inject(), without opening a real
// port or requiring the full .env. Everything below "Start" only runs when
// this file is executed directly (`node server.js`), not when imported.
...
export const DIST_DIR = path.join(__dirname, '..', 'web', 'dist');
const SESSION_TTL_DAYS = 30;
const SESSION_COOKIE = 'sid';
const STATE_COOKIE = 'oauth_state';

// ── App ──────────────────────────────────────────────────────────────────
export async function buildApp({ pool, config, distDir = DIST_DIR, logger = true }) {
  const { GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, ALLOWED_GITHUB_ID, SESSION_SECRET, PUBLIC_ORIGIN } = config;
...
// ── Start ──────────────────────────────────────────────────────────────────
// Only runs when this file is executed directly (`node server.js`), not when
// buildApp() is imported by tests.
async function main() {
  const {
    GITHUB_CLIENT_ID,
  ...
  const app = await buildApp({
    pool,
    config: { GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, ALLOWED_GITHUB_ID, SESSION_SECRET, PUBLIC_ORIGIN },
  });
